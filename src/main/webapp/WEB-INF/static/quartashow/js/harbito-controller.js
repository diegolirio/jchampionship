...
$(function() {
	...
	var form = $('#formHarbito');
	
	$('.saveHarbito').click(function(e) {
		e.preventDefault();		
		...
		$.post(	form.attr('action'),
				...
				function(data, statusText, response) {
					if(response.status == 201) {
						...
						closeWindowPopupStatusNormal(response.status, 'Harbito Criado com sucesso', response.responseJSON.id, response.responseJSON.nome, 'id_harbitos');
					} else {
